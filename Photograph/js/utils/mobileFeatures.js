 ...
 * }
 */
export const isInWebView = () => !!window.inAppWebview;
 ...
 */
export const callNative = (type, data) => {
...
if (typeof window !== "undefined") {
  window.handleNativeResponse = (response) => {
    const callback = window._nativeCallbacks?.[response.requestId];
    if (callback) {
      if (response.success) {
      ...
      delete window._nativeCallbacks[response.requestId];
    }
  };
}
 ...
 *
 * @param {string} dataURL - Base64 image data URL (e.g., "data:image/jpeg;base64,/9j/4AAQ...")
 * @param {string} filename - Filename with extension (e.g., "photo.jpg", "image.png")
 * @returns {Promise<boolean>} Promise resolving to true if save succeeded, false otherwise
 *
 * Use cases:
...
export const saveImageToDevice = async (
  ...
  if (isInWebView()) {
    ...
    try {
      const result = await callNative("saveImageToGallery", {
