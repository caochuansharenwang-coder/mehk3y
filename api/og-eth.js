// /api/og-eth — Dynamic OG image for /eth, with live price + 24h + ETH/BTC + Gas.
// 1200x630 PNG.
...
async function fetchData(req) {
  ...
  return {
    ...
    chg:    price?.chg ?? null,
    ethbtc: price?.ethbtc ?? null,
    gas:    gas?.gas ?? gas?.gwei ?? gas?.fast ?? null,
  };
}
...
export default async function handler(req) {
  ...
  const card = el('div', {
    ...
    el('div', { style: { display: 'flex', alignItems: 'center', gap: 14, fontSize: 14, color: colors.dim, fontFamily: 'Geist Mono', letterSpacing: '0.14em', textTransform: 'uppercase', zIndex: '1' } },
      el('div', { style: { width: 8, height: 8, borderRadius: 99, background: colors.purple, boxShadow: '0 0 12px ' + colors.purple } }),
      el('div', null, 'Mehk3y / eth dashboard')
    ),
    el('div', { style: { display: 'flex', flexDirection: 'column', zIndex: '1' } },
      el('div', { style: { display: 'flex', alignItems: 'baseline', gap: 22 } },
        el('div', { style: { fontSize: 76, fontWeight: 900, color: colors.purple, letterSpacing: '-0.045em', lineHeight: 1, fontFamily: 'Geist' } }, 'ETH'),
    ...
    el('div', { style: { display: 'flex', gap: 14, zIndex: '1' } },
      el('div', { style: { display: 'flex', flex: 1, flexDirection: 'column', padding: '18px 22px', border: '1px solid ' + colors.border, background: colors.surface, borderRadius: 14 } },
        el('div', { style: { fontSize: 12, color: colors.dim, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 500 } }, 'ETH / BTC'),
        el('div', { style: { fontSize: 38, fontWeight: 700, letterSpacing: '-0.02em', marginTop: 8, fontFamily: 'Geist Mono', color: colors.blue } }, data.ethbtc != null ? data.ethbtc.toFixed(4) : '—')
      ),
      el('div', { style: { display: 'flex', flex: 1, flexDirection: 'column', padding: '18px 22px', border: '1px solid ' + colors.border, background: colors.surface, borderRadius: 14 } },
        el('div', { style: { fontSize: 12, color: colors.dim, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 500 } }, 'gas · gwei'),
        el('div', { style: { fontSize: 38, fontWeight: 700, letterSpacing: '-0.02em', marginTop: 8, fontFamily: 'Geist Mono', color: colors.orange } }, data.gas != null ? (data.gas < 10 ? data.gas.toFixed(1) : Math.round(data.gas).toString()) : '—')
      ),
      el('div', { style: { display: 'flex', flex: 2, flexDirection: 'column', padding: '18px 22px', border: '1px solid ' + colors.border, background: colors.surface, borderRadius: 14 } },
        ...
        el('div', { style: { fontSize: 22, marginTop: 8, color: colors.text2, fontWeight: 500, letterSpacing: '-0.005em' } }, '200WMA · Staking · BMNR mNAV')
      )
    ),
    el('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', zIndex: '1', fontFamily: 'Geist Mono', fontSize: 14, color: colors.dim, letterSpacing: '0.08em' } },
      el('div', { style: { color: colors.text, fontWeight: 700, fontSize: 20, fontFamily: 'Geist', letterSpacing: '-0.01em' } }, 'Mehk3y.com/eth'),
