...
// 放行名单: 带流量的搜索引擎 + 社交分享预览爬虫(否则分享链接没缩略图)。
// 这些即使命中下面的拦截规则也不拦。
const ALLOWED_BOT_RE = /googlebot|google-inspectiontool|storebot-google|google-site-verification|mediapartners-google|bingbot|bingpreview|adidxbot|msnbot|slurp|duckduckbot|baiduspider|yandex|applebot|sogou|360spider|haosou|facebookexternalhit|facebookcatalog|twitterbot|telegrambot|whatsapp|linkedinbot|slackbot|discordbot|pinterest|skypeuripreview|vercel|uptimerobot/i;

// 明确拦截名单: AI 训练/检索爬虫、SEO 抓站、以及常见脚本客户端(伪装成功的拦不住, 但能挡掉绝大多数低质爬虫)。
const BLOCKED_BOT_RE = /gptbot|oai-searchbot|chatgpt-user|claude-?bot|claude-web|anthropic-ai|cohere-ai|ccbot|google-extended|perplexitybot|youbot|amazonbot|bytespider|imagesiftbot|diffbot|omgili|webzio|semrushbot|ahrefsbot|mj12bot|dotbot|blexbot|dataforseo|seekport|serpstatbot|megaindex|barkrowler|zoominfobot|petalbot|bytedance|magpie-crawler|scrapy|python-requests|python-urllib|aiohttp|go-http-client|node-fetch|axios|got \(|curl\/|wget|libwww-perl|java\/|jakarta|okhttp|httpclient|headlesschrome|phantomjs|puppeteer|playwright/i;
...
// 是否应当拦截: 放行名单优先; 否则命中黑名单或泛爬虫特征即拦。空 UA 也拦。
...
export async function recordVisit(entry) {
  ...
  // 每日访问量(趋势图用)
  commands.push(['HINCRBY', 'analytics:daily', day, 1]);

  // 单 IP 访问轨迹: 计数 + 最近 100 条访问, 30 天过期防膨胀
  if (entry.ip && entry.ip !== 'unknown') {
    commands.push(['ZINCRBY', 'analytics:ip-counts', 1, entry.ip]);
    commands.push(['LPUSH', `analytics:ip:${entry.ip}`, eventJson]);
    commands.push(['LTRIM', `analytics:ip:${entry.ip}`, 0, 99]);
    commands.push(['EXPIRE', `analytics:ip:${entry.ip}`, 60 * 60 * 24 * 30]);
  }

  return redisPipeline(commands);
}

// 最近 N 天的每日访问量与独立 IP, 供趋势图使用。
export async function readTrend(days = 90) {
  ...
  const commands = [['HMGET', 'analytics:daily', ...list]];
  for (const day of list) commands.push(['PFCOUNT', `analytics:unique:ips:${day}`]);

  const res = await redisPipeline(commands);
  if (!res.ok) return [];
  const results = res.results.map((item) => item?.result);
  const visits = results[0] || [];
  return list.map((day, idx) => ({
    day,
    visits: Number(visits[idx] || 0),
    uniqueIps: Number(results[1 + idx] || 0),
  }));
}

// 单个 IP 的访问明细 (最近若干条)。
export async function readIpTrail(ip, limit = 200) {
  const cfg = getRedisConfig();
  if (!cfg.ready || !ip) return [];
  const res = await redisPipeline([['LRANGE', `analytics:ip:${ip}`, 0, Math.max(0, Math.min(limit, 200) - 1)]]);
  if (!res.ok) return [];
  return (res.results[0]?.result || []).map(safeJson).filter(Boolean);
}
...
export async function readAnalytics(limit = 300) {
  ...
  const res = await redisPipeline([
    ...
    ['ZREVRANGE', 'analytics:ip-counts', 0, 49, 'WITHSCORES'],
  ]);
  ...
  const topIps = zsetPairs(values[19]);
