...
const PBKDF2_PATTERN = /^pbkdf2-sha256\$(\d{5,7})\$([a-f0-9]{32,128})\$([a-f0-9]{64})$/;
...
function authConfig() {
  ...
  const pbkdf2 = passwordHash.match(PBKDF2_PATTERN);
  const legacySha256 = /^[a-f0-9]{64}$/.test(passwordHash);
  return {
    passwordHash,
    pbkdf2,
    legacySha256,
    sessionSecret,
    ready: Boolean(pbkdf2 || legacySha256) && sessionSecret.length >= 32,
  };
}
...
export async function passwordIsValid(password) {
  ...
  if (cfg.pbkdf2) {
    const iterations = Number(cfg.pbkdf2[1]);
    const salt = hexToBytes(cfg.pbkdf2[2]);
    const expected = cfg.pbkdf2[3];
    const key = await crypto.subtle.importKey(
      'raw',
    ...
    return timingSafeEqual(bytesToHex(derived), expected);
  }

  // Temporary compatibility for installations that have not rotated yet.
  // New deployments should use the PBKDF2 record format documented in README.
  return timingSafeEqual(await sha256(password || ''), cfg.passwordHash);
}
