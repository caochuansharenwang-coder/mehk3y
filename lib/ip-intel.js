...
function geoSources(ip) {
  ...
  return [
    ...
    async () => {
      const d = await tryJSON(`https://ipwho.is/${encodedIp}`, 2800);
    ...
    async () => {
      const d = await tryJSON(`https://ipinfo.io/${encodedIp}/json`, 2800);
    ...
    async () => {
      const d = await tryJSON(`https://free.freeipapi.com/api/json/${encodedIp}`, 2800);
