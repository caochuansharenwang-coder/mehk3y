// lib/ip-intel.js — 共享的 IP 情报核心: 多源地理定位 + proxycheck 风险 + 信任评分。
//
// 同一套逻辑被两处复用:
...
// Claude 严格风控/封禁这些地区, 无论 IP 多干净都直接判 0 分。与 ip.js 保持一致。
export const CLAUDE_RESTRICTED_CC = {
  CN: '中国大陆', HK: '香港', MO: '澳门',
...
// Claude AI 信任评分 (0–100)。与 ip.js 的 trustScore 保持同一套权重。
...
// 查一个 IP 的完整情报。返回标准化对象 (含 trustScore / claudeRisk)。
export async function lookupIp(ip, { proxyKey = '' } = {}) {
  ...
  const cc = (base.countryCode || '').toUpperCase();
  return {
    ...base,
    trustScore: score,
    trustTier: trustTier(score),
    claudeRisk: (CLAUDE_RESTRICTED_CC[cc] || score < 50) ? 'high' : (score < 80 ? 'medium' : 'low'),
    restrictedRegion: CLAUDE_RESTRICTED_CC[cc] || null,
  };
}
